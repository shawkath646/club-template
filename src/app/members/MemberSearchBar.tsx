...
import { memberPublicSearchBarResult } from "@/backend/members";
import { MemberPublicSearchBarResultType } from "@/types";

export default function SearchBar() {
    const [searchValue, setSearchValue] = useState("");
    const [searchResult, setSearchResult] = useState<MemberPublicSearchBarResultType[]>([]);

    const debouncedSearch = debounce(async (query: string) => {
        if (query.length >= 3) {
            const result = await memberPublicSearchBarResult(query);
            ...
            <div className="relative w-full max-w-[500px]">
                <div className="relative flex items-center space-x-2">
                    <ComboboxInput
                        ...
                        placeholder="NBC ID, Name, Institute"
                    ...
                    {searchValue.length >= 3 && false ? (
                        <Link
                            href={`${process.env.NEXT_PUBLIC_APP_BASE_URL}/members?search=${searchValue}`}
                            ...
                            {searchResult.length > 0 ? (
                                searchResult.map((result, index) => (
                                    <ComboboxOption
                                        key={index}
                                        as={Link}
                                        href={`${process.env.NEXT_PUBLIC_APP_BASE_URL}/members?page=${result.pageNumber}`}
